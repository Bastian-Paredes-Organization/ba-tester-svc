...
For the project to work correctly, your folder structure should look like this:
    ...
    │   └── /src
    │
    └── ba-tester-ui   ← IMPORTANT (must be a sibling folder, not inside backend)

⚠️ **The `ba-tester-ui` folder must NOT be inside the backend project.**  
It needs to be at the same level (as a sibling directory), because the `frontend` service mounts it as a volume using a relative path (`../ba-tester-ui`).
...
* A `.npmrc` file is required in **both the backend and frontend folders**.  
  This file is **not included in the repository** and must be requested from another developer on the team.
